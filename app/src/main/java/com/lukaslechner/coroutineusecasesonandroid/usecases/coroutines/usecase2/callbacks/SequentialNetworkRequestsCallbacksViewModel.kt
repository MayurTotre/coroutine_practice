                ...
                if (response.isSuccessful){
                    ...
                    getAndroidFeaturesCall.enqueue(object: Callback<VersionFeatures>{
                        override fun onResponse(
                            ...
                            if(response.isSuccessful){
                                val featuresOfLatestVersions = response.body()
                                uiState.value = UiState.Success(featuresOfLatestVersions!!)
                            }else{
                                uiState.value = UiState.Error("Something Unexpected Happened!")
                            }
                        }

                        override fun onFailure(error: Call<VersionFeatures>, t: Throwable) {
